...
admissionsProbs <- function() {
  ...
  p_admit <- sum(UCBAdmissions[1,,]) / sum(UCBAdmissions)
  
  # P(female and admit) = 557/4526 
  # (total admitted females / totalAdmissions)
  p_admitandFemale <- sum(UCBAdmissions[1,2,]) / sum(UCBAdmissions)
  ...
  # = 17 / 25
  p_femaleandDeptB <- sum(UCBAdmissions[,2,2]) / sum(UCBAdmissions)
  p_admitandFemaleandDeptB <- sum(UCBAdmissions[1,2,2]) / sum(UCBAdmissions)
  ...
  # = 202 / 593
  p_femaleandDeptC <- sum(UCBAdmissions[,2,3]) / sum(UCBAdmissions)
  p_admitandFemaleandDeptC <- sum(UCBAdmissions[1,2,3]) / sum(UCBAdmissions)
  p_admitGivenFemaleandDeptC <- p_admitandFemaleandDeptC / p_femaleandDeptC
  
  # P(Dept. C | female) == P(dept c & female) / P(female)
  # = 593 / 1835
  p_deptCandFemale <- sum(UCBAdmissions[,2,3]) / sum(UCBAdmissions)
  p_deptCGivenFemale <- p_deptCandFemale / p_female
  
  # P(Dept C. or Dept. D) = P(Dept C) + P(Dept D)
  # = (918 + 792) / 4526
  p_deptCorD <- sum(UCBAdmissions[,,3] + UCBAdmissions[,,4]) / sum(UCBAdmissions)
  ...
  # (17 + 202) / (25 + 593)
  p_femaleandDeptBorC <- sum(UCBAdmissions[,2,2] + UCBAdmissions[,2,3]) / sum(UCBAdmissions)
  p_admitandFemaleandDeptBorC <- sum(UCBAdmissions[1,2,2] + UCBAdmissions[1,2,3]) / sum(UCBAdmissions)
